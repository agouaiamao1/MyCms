...
/* $Id: Smarty_Compiler.Tool.php 3163 2009-06-17 14:39:24Z monte.ohrt $ */

/**
 * Template compiling Tool
 * @package Smarty
 */
class Smarty_Compiler extends Smarty {
        ...
        if (count($this->_plugins['prefilter']) > 0) {
            foreach ($this->_plugins['prefilter'] as $filter_name => $prefilter) {
                ...
                if ($prefilter[3] || is_callable($prefilter[0])) {
                    ...
                    $this->_trigger_fatal_error("[Extend] prefilter '$filter_name' is not implemented");
                }
            }
        }
        ...
        if (count($this->_plugins['postfilter']) > 0) {
            foreach ($this->_plugins['postfilter'] as $filter_name => $postfilter) {
                ...
                if ($postfilter[3] || is_callable($postfilter[0])) {
                    ...
                    $this->_trigger_fatal_error("Smarty Extend error: postfilter '$filter_name' is not implemented");
                }
            }
        }
    ...
    function _compile_compiler_tag($tag_command, $tag_args, &$output)
    {
        ...
        /*
         * Otherwise we need to load Extend file and look for the function
         * inside it.
         */
            ...
            if (!is_callable($plugin_func)) {
                $message = "Extend function $plugin_func() not found in $plugin_file\n";
                $have_function = false;
            } else {
        ...
        /*
         * True return value means that we either found a Extend or a
         * dynamically registered function. False means that we didn't and the
         * compiler should now emit code to load custom function Extend for this
         * tag.
         */
        if ($found) {
            if ($have_function) {
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        /*
         * Even though we've located the Extend function, compilation
         * happens only once, so the Extend will still need to be loaded
    ...
    function _compile_custom_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        /* declare Extend to be loaded on display of the template that
        ...
        for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i++) {
            ...
            if (empty($this->_plugins['modifier'][$_modifier_name])
                ...
                if ($this->security && !in_array($_modifier_name, $this->security_settings['MODIFIER_FUNCS'])) {
                    $this->_trigger_fatal_error("[Extend] (secure mode) modifier '$_modifier_name' is not allowed" , $this->_current_file, $this->_current_line_no, __FILE__, __LINE__);
                } else {
    ...
    /**
     * compiles call to Extend of type $type with name $name
        ...
        if (isset($this->_plugins[$type][$name])) {
            ...
            if (is_array($this->_plugins[$type][$name][0])) {
                return ((is_object($this->_plugins[$type][$name][0][0])) ?
                        ...
                        : (string)($this->_plugins[$type][$name][0][0]).'::'    /* Tool callback */
            ...
            /* Extend not loaded -> auto-loadable-Extend */
     ...
     * Extend. return php-code to reflect the transition.
     * @return string
     */
     ...
     * cacheable state with the end of the current Extend return
